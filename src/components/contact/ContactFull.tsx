          ...
          <div className="mx-auto mt-12 flex max-w-md flex-col gap-3">
            {contacts.map((c) => (
              <a
                ...
                className="group flex items-center justify-between gap-4 rounded-2xl border border-[var(--border-strong)] bg-[var(--glass)] px-5 py-4 text-start backdrop-blur transition hover:border-[var(--accent)]"
              >
                <span>
                  <span className="block font-mono text-[0.58rem] uppercase tracking-[0.24em] text-[var(--ink-faint)]">
                    <T en={c.label} ar={c.labelAr ?? c.label} />
                  </span>
                  <span className="mt-0.5 block text-[var(--ink)] transition group-hover:text-[var(--accent-deep)]">
                    {c.value}
                  </span>
                </span>
                <span aria-hidden className="text-[var(--accent)] transition group-hover:translate-x-1 rtl:-scale-x-100">
                  →
                </span>
              </a>
            ))}
            ...
            <Magnetic className="mt-2 w-full" strength={0.25}>
              ...
              >
                <T en={resume.label} ar={resume.labelAr} />
